    ...
    for system in range(0, size):
        ...
        for dif_system in range(0, size):
            if dif_system != system:
                distance = (math.pow(systems[system][0]-systems[dif_system][0], 2) +
...
def create_cube(map_bounds, size, separation_factor, systems):
    ...
    for i in range(0, size):
        systems.append([np.random.uniform(low=map_bounds[0][0]*separation_factor, high=map_bounds[1][0]*separation_factor, size=None),
                       np.random.uniform(
                           low=map_bounds[0][1]*separation_factor, high=map_bounds[1][1]*separation_factor, size=None),
                       np.random.uniform(low=map_bounds[0][2]*separation_factor, high=map_bounds[1][2]*separation_factor, size=None)])
    return systems
...
def create_sphere(size, separation_factor, systems):
    ...
    for i in range(0, size):
        ...
        system = ([np.random.uniform(low=1, high=147100.0*separation_factor, size=None),
                   np.random.uniform(low=0, high=6.283185307, size=None),
                   np.random.uniform(low=3.1415321467, high=6.23185307, size=None)])

        systems.append([system[0]*math.sin(system[1])*math.cos(system[2]),
...
def create_random_system(current_system, separation_factor):
    ...
    new_system_sc = ([np.random.uniform(low=3.2*separation_factor, high=16*separation_factor, size=None),
                      np.random.uniform(low=0, high=6.283185307, size=None),
                      np.random.uniform(low=3, high=6.23185307, size=None)])

    new_system_cc = ([new_system_sc[0]*math.sin(new_system_sc[1])*math.cos(new_system_sc[2]),
                      new_system_sc[0]*math.sin(new_system_sc[1]) *
                      math.sin(new_system_sc[2]),
                      new_system_sc[0]*math.cos(new_system_sc[1])])

    system_true_possition = [current_system[0]+new_system_cc[0],
...
def create_cluster(size, separation_factor):
    ...
    for i in range(size):
        selected_system_index = system_candidates[int(
            system_random_selection(system_candidates))]
        system_candidates = system_out(
            system_candidates, selected_system_index)
...
map_bounds = [[-34710.0, -21048.0, -13465.0],
              [34710.0, 21048.0, 13465.0]]  # de esquina a esquina, 85530.49

#pa_bounds = [[-32843.0,-10524.0,-5163.0],[9542.0,21048.0,1369.0]]
#gu_bounds = [[-34708.0,-2104.0,-11440.0],[-11650.5, -10524.0, 13114.0]]
#td_bounds = [[-11650.5, -19024.0, -8163.0],[33751.0, 246.0,-435]]
#co_bounds = [[13412.0, 246.0,-2134.0], [32844.0,19314.0,12512.0]]

systems = []
separation_factor = 1  # de esquina a esquina, 85530.49 * separation_factor
size = 10
...
print("Tiempo de viaje a ", speed, " Ly/h: ", d_mean /
      (24*speed), "dias --- ", d_mean/(24*365*speed), " Años")
print("-----")
print("Distancia de viaje de caso mediana: ", d_median, " Ly")
print("Tiempo de viaje a ", speed, " Ly/h: ", d_median /
      (24*speed), "dias --- ", d_median/(24*365*speed), " Años")
print("-----")
print("Menor distancia entre dos estrellas cercanas: ", d_min, " Ly")
print("Tiempo de viaje a ", speed, " Ly/h: ", d_min /
      (24*speed), "dias --- ", d_min/(24*365*speed), " Años")
print("-----")
print("Mayor distancia entre dos estrellas cercanas: ", d_max, " Ly")
print("Tiempo de viaje a ", speed, " Ly/h: ", d_max /
      (24*speed), "dias --- ", d_max/(24*365*speed), " Años")
print("-----")
print("Mayor distancia en linea recta entre dos estrellas: ", d_fut[0])
print("Tiempo de viaje a ", speed, " Ly/h de la mayor distancia: ",
      d_fut[0]/(24*speed), "dias --- ", d_fut[0]/(24*365*speed), " Años")
